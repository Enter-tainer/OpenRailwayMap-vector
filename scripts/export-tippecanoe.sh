...
#   LAYERS        - Which style layers to export: "all" (default) or "standard" (standard-only, smaller output)
...
SELECT
  ...
  traffic_mode,
  radio,
  (select nullif(array_to_string(array_agg(rt.osm_id || U&'\001E' || coalesce(rt.color, '') || U&'\001E' || coalesce(rt.name, '')), U&'\001D'), '') from route_line rl join routes rt on rl.route_id = rt.osm_id where rl.line_id = l.osm_id) as line_routes,
  (select count(*) from route_line rl join routes rt on rl.route_id = rt.osm_id where rl.line_id = l.osm_id) as route_count,
  wikidata,
  wikimedia_commons,
...
SELECT
  p.id, p.osm_id, osm_type, p.way,
  'platform' as feature, p.name,
  ...
  (select nullif(array_to_string(array_agg(rt.osm_id || U&'\001E' || coalesce(rt.color, '') || U&'\001E' || coalesce(rt.name, '')), U&'\001D'), '') from routes rt where rt.platform_ref_ids @> Array[p.osm_id]) as platform_routes
FROM platforms p
"
...
SELECT
  sp.id, sp.osm_id, sp.way, sp.name, sp.type, sp.ref, sp.local_ref,
  (select nullif(array_to_string(array_agg(rt.osm_id || U&'\001E' || coalesce(rt.color, '') || U&'\001E' || coalesce(rt.name, '')), U&'\001D'), '') from route_stop rs join routes rt on rs.route_id = rt.osm_id where rs.stop_id = sp.osm_id) as stop_position_routes
...
# Uses UNION ALL + JOIN routes pattern matching Martin's function
# to generate station_routes and references fields
...
FROM (
  SELECT *, UNNEST(route_ids) as route_id
  FROM grouped_stations_with_importance
  UNION ALL
  SELECT *, NULL as route_id
  FROM grouped_stations_with_importance
) gs
LEFT JOIN railway_operator ro
